function u_sequence = mpcSolSLPsemiMICP(A, B, umin, umax, x0, N, Q, R, radius, center, doPlot, nhat, K)
    if nargin < 9
        radius = 0;
        center = [0, 0];
    end
    ...
    baseConstraints = [0 <= u <= umax];
    objective = 0;
    x = x0;
    for k = 1:N
        for m = 1:M
            baseConstraints = [baseConstraints
                implies(on(k, m), [umin <= u(k, m)])
                implies(~on(k, m), [u(k, m) == 0])
            ];
        end
        baseConstraints = [baseConstraints,
            sum(on(k, :)) <= K;
        ...
        x = A*x + B*uk;
        objective = objective + x'*Q*x + uk'*R*uk + sum(on(k, :));
        
    end
    ops = sdpsettings('solver','gurobi');
    ops.gurobi.TimeLimit = 10;
    optimize([baseConstraints, x0_ == x0], objective, ops);
    ...
    if any(inCircle)
        ...
        while norm(lastu - u_sequence) > tol && iter <= niter
            ...
            optimize([extendedConstraints, x0_ == x0], objective, ops);
            for k=1:N
